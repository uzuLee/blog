        ...
        try {
            ...
            if (fmMatch) {
                ...
                if (fm) {
                    ...
                    if (fm.date) meta.date = String(fm.date);
                } else {
                }
            } else {
            }
            ...
            // Build title index (with normalized whitespace)
            const normalizedTitle = meta.title.toLowerCase().replace(/\s+/g, ' ');
            const normalizedSlug = meta.slug.toLowerCase().replace(/\s+/g, ' ');

            titleIndex.set(meta.title.toLowerCase(), meta.path);
            titleIndex.set(normalizedTitle, meta.path);
            titleIndex.set(meta.slug.toLowerCase(), meta.path);
            titleIndex.set(normalizedSlug, meta.path);

            for (const alias of meta.aliases) {
                const normalizedAlias = alias.toLowerCase().replace(/\s+/g, ' ');
                titleIndex.set(alias.toLowerCase(), meta.path);
                titleIndex.set(normalizedAlias, meta.path);
            }
        } catch (err) {
        }
    });
...
export function resolveByTitle(currentFolder, titleOrAlias) {
    ...
    // Global exact match
    if (titleIndex.has(normalizedKey)) {
        return titleIndex.get(normalizedKey);
    }
    ...
    for (const meta of metaCache.values()) {
        ...
        const titleMatch = normalizedTitle === normalizedKey || meta.title.toLowerCase() === key;
        const slugMatch = normalizedSlug === normalizedKey || meta.slug.toLowerCase() === key;
        const aliasMatch = meta.aliases.some(a => {
            const normalizedAlias = a.toLowerCase().replace(/\s+/g, ' ');
            return normalizedAlias === normalizedKey || a.toLowerCase() === key;
        });
...
export function parseLinksFrom(path) {
    ...
    for (const match of raw.matchAll(WIKI_RE)) {
        ...
        if (target.includes('/')) {
            let resolvedPath;
            if (isRelative) {
                resolvedPath = resolveRelativePath(path, target);
            } else {
                resolvedPath = target;
            }
        ...
        if (!resolved) {
            resolved = resolveByTitle(meta.folder, target);
        }
