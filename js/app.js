...
const renderBookmarksPanel = async (panelEl, appState, app) => {
    panelEl.innerHTML = '<div class="loading-placeholder">Loading bookmarks...</div>';
...
async function initAuth() {
    state.currentUser = await github.getUser();
    renderAuthUI();
    ...
    if (state.currentUser) {
        $.authContainer.innerHTML = `
            <div class="user-info">
                ...
                <span>${state.currentUser.login}</span>
            </div>
            <button id="btn-logout" class="btn ghost btn-logout">Logout</button>
        `;
    } else {
        $.authContainer.innerHTML = `
            <button id="btn-login" class="btn ghost btn-login-header" title="Login with GitHub">
...
async function loadComments(docPath, docTitle) {
    // Clear old data
    $.likesContainer.innerHTML = '';
    $.commentsList.innerHTML = '';

    try {
        const issue = await github.findIssueForPost(docPath);
        state.currentIssue = issue;
        
        renderLikes(issue, docPath, docTitle); // Always render likes UI

        if (issue) {
            const comments = await github.getComments(issue.number);
            renderComments(comments);
        } else {
            $.commentsList.innerHTML = '<p class="comments-placeholder">No comments yet. Be the first to comment!</p>';
        }
        renderCommentForm(docPath, docTitle);
...
function renderLikes(issue, docPath, docTitle) {
    const likeCount = issue?.reactions?.['+1'] || 0;
    $.likesContainer.innerHTML = `
        <button id="btn-like" class="btn btn-like" title="Like this post">
            <span>👍</span>
            <span class="like-count">${likeCount}</span>
        </button>
    `;

    const likeButton = $.likesContainer.querySelector('#btn-like');
    ...
    likeButton.addEventListener('click', () => handleLikeClick(issue, docPath, docTitle), { once: true });
}

async function handleLikeClick(issue, docPath, docTitle) {
    const likeButton = $.likesContainer.querySelector('#btn-like');
    likeButton.disabled = true;
    ...
    try {
        ...
        if (!issueNumber) {
            const newIssue = await github.createIssue(docPath, docTitle);
            state.currentIssue = newIssue;
            issueNumber = newIssue.number;
        }

        await github.likePost(issueNumber);
        
        const countSpan = likeButton.querySelector('.like-count');
        const newCount = parseInt(countSpan.textContent, 10) + 1;
        countSpan.textContent = newCount;
        likeButton.title = 'You liked this post';
...
function renderComments(comments) {
    ...
    $.commentsList.innerHTML = comments.map(comment => {
        const isOwner = comment.user.login === CONFIG.githubUser;
        const ownerBadge = isOwner ? '<span class="owner-badge">Owner</span>' : '';
        const ownerClass = isOwner ? 'comment-by-owner' : '';
            ...
            <div class="comment-item ${ownerClass}">
                ...
                <div class="comment-body">
                    <div class="comment-header">
                        <div>
                            <a href="${comment.user.html_url}" target="_blank" rel="noopener noreferrer" class="comment-author">${comment.user.login}</a>
                            ${ownerBadge}
                        </div>
                        <span>${getTimeAgo(new Date(comment.created_at).getTime())}</span>
                    </div>
...
function renderCommentForm(docPath, docTitle) {
    if (state.currentUser) {
        // User is logged in, show the active form
        $.commentFormContainer.innerHTML = `
            <div class="discord-form">
                <button id="btn-attach-file" class="btn-attach" title="Attach file or image (Ctrl+V to paste)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"></path>
                    </svg>
                </button>
                <textarea id="comment-textarea" placeholder="Message (Ctrl+V to paste images)..."></textarea>
                <input type="file" id="file-input" accept="image/*,video/*,.pdf,.zip,.txt,.md" style="display:none;" multiple>
                <button id="btn-submit-comment" class="btn ghost btn-send" title="Send">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        const textarea = $.commentFormContainer.querySelector('#comment-textarea');
        const fileInput = $.commentFormContainer.querySelector('#file-input');
        const attachBtn = $.commentFormContainer.querySelector('#btn-attach-file');

        // File button click - show info about file uploads
        attachBtn.addEventListener('click', () => {
            const message = 'File uploads through the API are limited. You can:\n\n' +
                '1. Paste images directly (Ctrl+V)\n' +
                '2. Add markdown links manually: ![image](url)\n' +
                '3. Upload files on GitHub Issues page after posting';
            alert(message);
        });

        // File selection
        fileInput.addEventListener('change', (e) => handleFileUpload(e.target.files, textarea));
        ...
        textarea.addEventListener('paste', (e) => {
            ...
            for (const item of items) {
                if (item.type.indexOf('image') !== -1) {
                    ...
                    if (file) handleFileUpload([file], textarea);
                }
            }
        });
        ...
        textarea.addEventListener('dragover', (e) => {
            e.preventDefault();
            textarea.style.borderColor = 'var(--c-primary)';
        });
        ...
        textarea.addEventListener('drop', (e) => {
            ...
            if (e.dataTransfer?.files.length) {
                handleFileUpload(e.dataTransfer.files, textarea);
            }
        });

        $.commentFormContainer.querySelector('#btn-submit-comment').addEventListener('click', () => handleCommentSubmit(docPath, docTitle));
    } else {
        // User is not logged in, show a disabled-looking form that prompts login on click.
        $.commentFormContainer.innerHTML = `
            <div id="login-prompt-form" class="discord-form disabled-prompt">
                <button class="btn-attach" disabled title="Login to attach files">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"></path>
                    </svg>
                </button>
                <textarea placeholder="Log in to leave a comment..." disabled></textarea>
                <button class="btn ghost btn-send" disabled title="Send">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <line x1="22" y1="2" x2="11" y2="13"></line>
                        <polygon points="22 2 15 22 11 13 2 9 22 2"></polygon>
                    </svg>
        ...
        `;
        const loginPromptForm = $.commentFormContainer.querySelector('#login-prompt-form');
        loginPromptForm.addEventListener('click', (e) => {
            e.preventDefault();

            // Add error shake to comment form
            loginPromptForm.classList.add('error-shake');
            setTimeout(() => loginPromptForm.classList.remove('error-shake'), 500);

            // Add pulse to login button
            const loginBtn = document.getElementById('btn-login');
            if (loginBtn) {
                loginBtn.classList.add('pulse-blue');
                setTimeout(() => loginBtn.classList.remove('pulse-blue'), 1000);
            }

            // Don't redirect - just show visual feedback
        });
    }
}

async function handleFileUpload(files, textarea) {
    if (!files || files.length === 0) return;

    // For pasted images, add markdown placeholder
    // Note: GitHub API doesn't support direct file uploads via comments
    // Users need to paste images or add URLs manually
    ...
    for (const file of files) {
        ...
        if (file.type.startsWith('image/')) {
            // For images pasted from clipboard, GitHub will handle the upload when the comment is submitted
            markdown = `\n![Pasted image](paste-your-image-here)\n`;

            // Show helpful message
            setTimeout(() => {
                alert('Image detected! After posting your comment, you can edit it on GitHub to upload the actual image.');
            }, 100);
        } else {
            markdown = `\n[📎 ${file.name}](add-file-url-here)\n`;
        }
...
async function handleCommentSubmit(docPath, docTitle) {
    const textarea = $.commentFormContainer.querySelector('#comment-textarea');
    const body = textarea.value.trim();
    if (!body) return;

    const submitButton = $.commentFormContainer.querySelector('#btn-submit-comment');
    ...
    try {
        ...
        if (!issue) {
            issue = await github.createIssue(docPath, docTitle);
            state.currentIssue = issue;
        }

        const newComment = await github.postComment(issue.number, body);
        textarea.value = '';
