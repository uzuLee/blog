    ...
    panelEl.querySelectorAll('a[data-scroll-to]').forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (targetEl && contentArea) {
                ...
                const targetScrollPosition = contentArea.scrollTop + (targetRect.top - contentAreaRect.top) - offset;
...
const renderBacklinksPanel = (panelEl, appState, app) => {
    if (!appState.currentPath) {
        panelEl.innerHTML = '<div class="toc-placeholder">No document selected</div>';
        return;
    }
...
const updateBacklinksPanel = (panelEl, appState, app) => {
    ...
    if (backlinks.length === 0) {
        panelEl.innerHTML = '<div class="toc-placeholder">No backlinks found</div>';
        return;
    }
...
function renderHomePage() {
    ...
    HOME_CARDS.forEach(card => {
        ...
        if (card.bindEvents) {
            ...
            card.bindEvents(cardElement, metaCache, renderTagResultsPage, openDocument);
        }
        if (card.init) {
            card.init(state, $);
        }
...
function closeGraph() {
    ...
    // Clean up 3D Force Graph and WebGL context
    if (state.forceGraph) {
        if (state.forceGraph.pauseAnimation) {
            state.forceGraph.pauseAnimation();
        }
        // Dispose of renderer and scene to prevent WebGL context issues
        if (state.forceGraph.renderer) {
            state.forceGraph.renderer().dispose();
        }
        if (state.forceGraph.scene) {
...
function renderGraph(graphData = null) { // Signature corrected
    console.log('[DEBUG renderGraph] Starting 2D graph rendering', { graphData, hasGraphData: !!graphData });
    ...
    try {
        ...
        console.log('[DEBUG renderGraph] Elements created', {
            totalElements: elements.length,
            nodes: gData.nodes.length,
            links: gData.links.length
        });
        ...
        const layoutOptions = {
            ...
            padding: 80,
                    ...
                    console.log('[DEBUG renderGraph] Initializing Cytoscape', {
                    ...
                    // Run the layout
                    console.log('[DEBUG renderGraph] Running layout', layoutOptions);

                    state.cy.layout(layoutOptions).run();

                    console.log('[DEBUG renderGraph] 2D graph rendering complete');
                    ...
                    console.error('[ERROR renderGraph] 2D graph rendering failed:', error);
                    $.graphContainer.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--c-text-secondary);">Error rendering graph: ${error.message}</div>`;
                }
            ...
            function render3DGraph(graphData = null) { // Signature corrected
                ...
                state.forceGraph.controls().panSpeed = 0.2;
                state.forceGraph.controls().enableDamping = true;
                state.forceGraph.controls().dampingFactor = 0.1;
