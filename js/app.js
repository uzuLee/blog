...
import { languages, languageMetadata } from "./i18n/index.js";
...
// Utilities
// ============================================
function updateStatus(msg, key = null, params = {}) {
  // Store the status key and params for language switching
  if (key) {
    state.currentStatusKey = key;
    state.currentStatusParams = params;
  }
  if ($.status) $.status.textContent = msg;
}

function refreshStatus() {
  // Re-apply current status with new language
  if (state.currentStatusKey) {
    let statusText = t(state.currentStatusKey);
    // Replace any parameters
    Object.entries(state.currentStatusParams || {}).forEach(([key, value]) => {
      statusText = statusText.replace(`{${key}}`, value);
    });
...
function populateLanguageSelector() {
  // Dynamically populate language selector from i18n metadata
  ...
  // Add options from languageMetadata
  Object.entries(languageMetadata).forEach(([code, metadata]) => {
    const option = document.createElement('option');
    option.value = code;
    option.textContent = `${metadata.nativeName} (${metadata.name})`;
    $.languageSelect.appendChild(option);
  });

  // Restore selected value if it still exists
  if (currentValue && languageMetadata[currentValue]) {
...
function updateSettingsText() {
  ...
  if (labelFontSize) {
    labelFontSize.textContent = t("settings.font_size");
  }

  const labelFontFamily = document.getElementById("label-font-family");
  if (labelFontFamily) {
    labelFontFamily.textContent = t("settings.font_family");
  }

  const labelLanguage = document.getElementById("label-language");
  if (labelLanguage) {
    labelLanguage.textContent = t("settings.language");
  }

  const labelResetSettings = document.getElementById("label-reset-settings");
  if (labelResetSettings) {
    labelResetSettings.textContent = t("settings.reset");
  }

  const btnSettingsClose = document.getElementById("btn-settings-close");
  if (btnSettingsClose) {
    btnSettingsClose.title = t("settings.close");
  }
}
...
function setLanguage(lang) {
  ...
  updateBlogName();
  updateSettingsText();
  refreshStatus(); // Update status bar with new language
...
function scrollToAnchor(anchor) {
  ...
  // Try to find element by ID (for headings)
  targetElement = $.docContent.querySelector(`#${CSS.escape(anchor)}`);

  // If not found, try finding by heading text
  if (!targetElement) {
    ...
    for (const heading of headings) {
      const headingId = heading.textContent.toLowerCase()
        .replace(/[^\w\s-]/g, '')
        .replace(/\s+/g, '-');
      if (headingId === anchor.toLowerCase() || heading.id === anchor) {
...
function showHome() {
  ...
  updateStatus(t("status.ready"), "status.ready");
}
...
function showFolderPage(folderPath) {
  ...
  updateStatus(t("status.viewing_folder").replace("{folder}", folderPath), "status.viewing_folder", { folder: folderPath });
}
...
function showResultsPage() {
  ...
  updateActiveFileInList();
  renderPanels(null); // Clear panels for non-document pages
  updateStatus(t("status.viewing_search_results"), "status.viewing_search_results");
}
...
// Document Rendering
// ============================================ 
async function openDocument(path, anchor = null) {
  handleQuickViewHide();
  cleanupHeroCard(); // Clean up hero card rotation interval
  ...
  // Parse path and anchor if combined
  if (path.includes('::')) {
  ...
  if (state.currentPath === path && !anchor) return;
  ...
  setTimeout(async () => {
    ...
    let content = fileCache.get(path);
    if (!content) {
      updateStatus(t("status.loading").replace("{title}", meta.title), "status.loading", { title: meta.title });
      content = await fetchRaw(path, meta.sha);
      fileCache.set(path, content);
    }
    ...
    // Update hash with anchor if provided
    const fullHash = anchor ? `${encodeURIComponent(path)}::${anchor}` : encodeURIComponent(path);
    window.location.hash = fullHash;
    updateStatus(t("status.viewing").replace("{title}", meta.title), "status.viewing", { title: meta.title });
...
function resetSettings() {
    ...
    // Show confirmation
    updateStatus(t("status.settings_reset"), "status.settings_reset");
    setTimeout(() => updateStatus(t("status.ready"), "status.ready"), 2000);
}
...
function bindEvents() {
  ...
  window.addEventListener("hashchange", () => {
    ...
    if (path && metaCache.has(path)) {
      openDocument(path, anchor);
    } else if (decodedHash && metaCache.has(decodedHash)) {
      openDocument(decodedHash);
    } else {
      showHome();
    }
  });
...
async function init() {
  ...
  try {
    updateStatus(t("status.initializing"), "status.initializing");
    ...
    // Populate language selector dynamically from i18n metadata
    ...
    updateStatus(t("status.fetching_files"), "status.fetching_files");

    state.allFiles = await fetchGitHubTree();

    updateStatus(t("status.indexing"), "status.indexing");

    await indexFiles(state.allFiles, fetchRaw);

    updateStatus(t("status.building_graph"), "status.building_graph");

    buildGraph({ type: "global" });

    updateStatus(t("status.building_search_index"), "status.building_search_index");
    ...
    updateStatus(t("status.error").replace("{message}", err.message), "status.error", { message: err.message });
  }
}
